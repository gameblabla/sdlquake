...
{
    if (cl.gametype == GAME_DEATHMATCH)
	Draw_TransPic(x /*+ ((spacing_x_res)>>1) */ ,
		      y + (vid.height - SBAR_HEIGHT), pic);
    else
	Draw_TransPic(x + ((spacing_x_res) >> 1),
		      y + (vid.height - SBAR_HEIGHT), pic);
...
{
    if (cl.gametype == GAME_DEATHMATCH)
	Draw_Character(x /*+ ((spacing_x_res)>>1) */  + 4,
...
Sbar_DrawScoreboard(void)
{
	...
	Draw_Fill(x * 8 + 10 + ((spacing_x_res) >> 1),
...
// PGM 01/19/97 - team color drawing
	...
	Sbar_DrawPic(0, 0, sb_face_invis_invuln);
	return;
    }
    if (cl.stats[STAT_ITEMS] & IT_QUAD) {
	Sbar_DrawPic(0, 0, sb_face_quad);
	return;
    }
    if (cl.stats[STAT_ITEMS] & IT_INVISIBILITY) {
	Sbar_DrawPic(0, 0, sb_face_invis);
	return;
    }
    if (cl.stats[STAT_ITEMS] & IT_INVULNERABILITY) {
	Sbar_DrawPic(0, 0, sb_face_invuln);
	return;
    }
	...
	anim = 0;
    Sbar_DrawPic(0, 0, sb_faces[f][anim]);
}
...
// time
    ...
    Sbar_IntermissionNumber(160-96, 64, dig, 3, 0);
    num = cl.completed_time - dig * 60;
    Draw_TransPic(234-96, 64, sb_colon);
    Draw_TransPic(246-96, 64, sb_nums[0][num / 10]);
    Draw_TransPic(266-96, 64, sb_nums[0][num % 10]);

    Sbar_IntermissionNumber(160-96, 104, cl.stats[STAT_SECRETS], 3, 0);
    Draw_TransPic(232-96, 104, sb_slash);
    Sbar_IntermissionNumber(240-96, 104, cl.stats[STAT_TOTALSECRETS], 3, 0);

    Sbar_IntermissionNumber(160-96, 144, cl.stats[STAT_MONSTERS], 3, 0);
    Draw_TransPic(232-96, 144, sb_slash);
    Sbar_IntermissionNumber(240-96, 144, cl.stats[STAT_TOTALMONSTERS], 3, 0);*/
    char tmp[256];
