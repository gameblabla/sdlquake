...
// bound field of view
	...
	// widescreen hack. put the two lines below back and remove the block underneath to return to normal fov setting

	//	r_refdef.fov_x = scr_fov.value;
	//	r_refdef.fov_y = CalcFov (r_refdef.fov_x, (float)r_refdef.vrect.width, (float)r_refdef.vrect.height);

	//widescreen fov from quakesrc.org

   if (scr_fov.value > 89 && scr_fov.value < 91)
   {
      // default fov
	...
	//         r_refdef.fov_y = 73.739797;
        // r_refdef.fov_x = CalcFov (73.739797, r_refdef.vrect.height, r_refdef.vrect.width);
         r_refdef.fov_y = 65;
         r_refdef.fov_x = CalcFov (65, r_refdef.vrect.height, r_refdef.vrect.width);
      }
      else
      {
         ...
         r_refdef.fov_x = 90;
         r_refdef.fov_y = CalcFov (90, r_refdef.vrect.width, r_refdef.vrect.height);
      }
   }
   else
   {
...
//		GpError("scr_drawdialog",3);
	...
	{
		GpError("scr_drawloading",3);
		SCR_DrawLoading ();
		Sbar_Draw ();
	}
	else if (cl.intermission == 1 && key_dest == key_game)
	{
		GpError("cl_intermission==1",3);
		Sbar_IntermissionOverlay ();
	}
	else if (cl.intermission == 2 && key_dest == key_game)
	{
		GpError("cl_intermission==2",3);
		Sbar_FinaleOverlay ();
		SCR_CheckDrawCenterString ();
	}
	else if (cl.intermission == 3 && key_dest == key_game)
	{
		GpError("cl_intermission==3",3);
		SCR_CheckDrawCenterString ();
	}
	else
	{
