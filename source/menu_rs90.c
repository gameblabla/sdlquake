...
M_Main_Draw(void)
{
	...
	M_Print (120-32, 20, "Single Player");
	M_Print (120-32, 20+16*1, "Options");
	M_Print (120-32, 20+16*2, "Quit");
	M_Print (120-48, 20+(16*m_main_cursor), ">");
}
...
M_Options_Draw(void)
{
    ...
    M_DrawPic((240 - pic->width) / 2, 4, pic);

    M_Print(8, height = 32, "Customize controls");
    M_Print(8, height += 8, "Go to console");
    M_Print(8, height += 8, "Reset to defaults");

    slider = (scr_viewsize.value - 30) / (120 - 30);
    M_Print(8, height += 8, "Screen size");
    M_DrawSlider(148, height, slider);

    slider = (1.0 - v_gamma.value) / 0.5;
    M_Print(8, height += 8, "Brightness");
    M_DrawSlider(148, height, slider);

    slider = (sensitivity.value - 1) / 10;
    M_Print(8, height += 8, "Mouse Speed");
    M_DrawSlider(148, height, slider);

    slider = bgmvolume.value;
    M_Print(8, height += 8, "CD Music Volume");
    M_DrawSlider(148, height, slider);

    slider = volume.value;
    M_Print(8, height += 8, "Sound Volume");
    M_DrawSlider(148, height, slider);

    M_Print(8, height += 8, "Always Run");
    M_DrawCheckbox(148, height, cl_run.value);

    M_Print(8, height += 8, "Invert Mouse");
    M_DrawCheckbox(148, height, m_pitch.value < 0);

    M_Print(8, height += 8, "Mouse Look");
    M_DrawCheckbox(148, height, m_freelook.value);

    M_Print(8, height += 8, "Lookspring");
    M_DrawCheckbox(148, height, lookspring.value);

    M_Print(8, height += 8, "Lookstrafe");
    M_DrawCheckbox(148, height, lookstrafe.value);

#ifdef QW_HACK
    M_Print(8, height += 8, "Use old status bar");
    M_DrawCheckbox(148, height, cl_sbar.value);

    M_Print(8, height += 8, "HUD on left side");
    M_DrawCheckbox(148, height, cl_hudswap.value);
#endif

    /*M_Print(8, height += 8, "Video Options");

    M_Print(8, height += 8, "Use Mouse");
    M_DrawCheckbox(148, height, _windowed_mouse.value);*/

    /* cursor */
    M_DrawCharacter(0, 32 + m_options_cursor * 8,
		    12 + ((int)(realtime * 4) & 1));
}
...
M_Keys_Draw(void)
{
	...
	if (keys[0] == K_UNKNOWN) {
	    M_Print(140-24, height, "???");
	} else {
	    const char *keyname = Key_KeynumToString(keys[0]);
	    M_Print(140-24, height, keyname);
	    if (keys[1] != K_UNKNOWN) {
		const int namewidth = strlen(keyname) * 8;
		keyname = Key_KeynumToString(keys[1]);
		M_Print(140-24 + namewidth + 8, height, "or");
		M_Print(140-24 + namewidth + 32, height, keyname);
	    }
	}
    }
    ...
    if (m_keys_bind_grab) {
	M_Print (0, 0, "Press button for this action");
	M_DrawCharacter(0, 16 + m_keys_cursor * 8, '=');
    } else {
	const int cursor_char = 12 + ((int)(realtime * 4) & 1);
	M_Print (0, 0, "Start=Change Select=Clear");
	M_DrawCharacter(0, 16 + m_keys_cursor * 8, cursor_char);
    }
}
...
M_Quit_Draw(void)
{
    ...
    M_DrawTextBox(56-40, 76, 24, 4);
    M_Print(64-40, 84, m_quit_message[0]);
    M_Print(64-40, 92, m_quit_message[1]);
    M_Print(64-40, 100, m_quit_message[2]);
    M_Print(64-40, 108, m_quit_message[3]);
}
...
M_SinglePlayer_Draw(void)
{
    ...
    M_DrawPic((240 - p->width) / 2, 4, p);
    M_DrawTransPic(48, 40, Draw_CachePic("gfx/sp_menu.lmp"));

    f = (int)(host_time * 10) % 6;

    M_DrawTransPic(24, 38 + m_singleplayer_cursor * 20,
